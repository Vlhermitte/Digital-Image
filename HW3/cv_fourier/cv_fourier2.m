...
for i=1:5
    ...
    % (frequency domain filter of the same size as the spectrum)
...
filter_size = size(X); % size of the filter should be the same as the spectrum
sigma = 20; % adjust sigma based on your requirements

% Create a Gaussian filter
[x_grid, y_grid] = meshgrid(-filter_size(2)/2 : filter_size(2)/2 - 1, ...
                             -filter_size(1)/2 : filter_size(1)/2 - 1);
A = exp(-(x_grid.^2 + y_grid.^2) / (2 * sigma^2));
...
for f = [fig-1,fig] %Visualize the reconstruction filter extent
    ...
    tmp_x = find(sum(S,2)>0);
    tmp_y = find(sum(S,1)>0);
    plot([tmp_x(1),tmp_x(end),tmp_x(end),tmp_x(1),tmp_x(1)], [tmp_y(1),tmp_y(1),tmp_y(end),tmp_y(end),tmp_y(1)], 'r');
    hold off
end
